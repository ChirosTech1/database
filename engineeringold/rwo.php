...
switch($_POST['submit'])
{
	...
	case "Next":
		...
		break;
	case "Approve":
		if($_POST['approve']=='ENG')
		{
			$wo = $_POST['no'];
			mysql_query("UPDATE wo SET eng = CURDATE() WHERE no = '$wo'");
		}
		else
		{
			?>
...
$sql = "SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date, rev AS worev FROM customer INNER JOIN wo ON customer.id = wo.custid WHERE wo.eng IS NULL ORDER BY wo.no LIMIT $limit,1";
...
?>
<h1>Engineering Work Order Release Form</h1>
