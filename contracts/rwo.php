...
switch($_POST['submit'])
{
	...
	case "Next":
		...
		break;
	case "Approve":
		if($_POST['approve']=='CONT')
		{
			$wo = $_POST['no'];
			mysql_query("UPDATE wo SET cont = CURDATE() WHERE no = '$wo'");
		}
		else
		{
			?>
...
$sql = "SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date, rev AS worev FROM customer INNER JOIN wo ON customer.id = wo.custid WHERE wo.cont IS NULL ORDER BY wo.no LIMIT $limit,1";
...
?>
<h1>Contracts Work Order Release Form</h1>
