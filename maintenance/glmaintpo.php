...
<script type="text/javascript" src="../script/maintpo.js"/></script>
</head>
<body>
<?php
//Database login
...
switch ($q)
{
	...
	case "Delete":
		...
		$totsql = "SELECT SUM(lp) AS lp, SUM(tax) AS tax, SUM(lp+tax) AS tot FROM poline WHERE no = '$linkvalue'";
		...
		mysql_query("UPDATE $ptable SET subtot='$subtotal', taxtot='$taxtotal', tot='$total' WHERE no='$linkvalue'");
...
?>
	...
	<input type="button" id="<?php echo $i;?>" name="delete" value="Delete" onclick="UpdateLine(this.value,this)"/>
	</tr>
<?php
}
	//Get Last Line Number for New Field
...
?>
	<tr>
	<td><input type="text" id="<?php echo $i;?>" name="line" size="2" value="<?php echo $maxline;?>" onchange="UpdateLine('New', this)"/></td>
	...
	<td><input type="text" id="<?php echo $i;?>" name="lp" size="10" onchange="UpdateLine('New', this)"/></td>
	</tr>
	</table>
	</form>
<?php
