...
switch($_POST['submit'])
{
	...
	case "Approve":
		...
		{
			$wo = $_POST['no'];
			mysql_query("UPDATE wo SET qc = CURDATE() WHERE no = '$wo'");
		}
		else
		{
			?>
...
$sql = "SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date, rev AS worev FROM customer INNER JOIN wo ON customer.id = wo.custid WHERE wo.qc IS NULL ORDER BY wo.no LIMIT $limit,1";
...
?>
<h1>Quality Control Work Order Release Form</h1>
