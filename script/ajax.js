...
function UpdateForm(q,live)
{
	...
	else if (q == 'New ' + formtype)
	{
		if(confirm("Are you sure you want to Add a new " + formtype + "?"))
		{
			...
			else if (q == "New Specification")
			{
				var spec = prompt ("What is the New Specification Number?");
				if(spec)
				{
					var type = prompt ("What is the Specification Type? (i.e. Customer, Military, International)","Military");
					if (type)
					{
						pinfo = "q="+q+"&spec="+spec+"&type="+type;
						AjaxCall(xhr,url,pinfo);
					}
					else
						window.close();
			...
			else if (q == "New Drawing")
			{
				var drawing = prompt ("What is the New Drawing Number?");
				if(drawing)
				{
					pinfo = "q="+q+"&drawing="+drawing;
					AjaxCall(xhr,url,pinfo);
				}	
			}
			...
			else if (q == "New Tooling")
			{
				var tooling = prompt ("What is the New Tooling Number?");
				if(tooling)
				{
					pinfo = "q="+q+"&tooling="+tooling;
					AjaxCall(xhr,url,pinfo);
				}	
			}
			else if (q == "New Drawing")
